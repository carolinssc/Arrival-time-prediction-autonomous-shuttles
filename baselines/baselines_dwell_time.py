...
def MeanRegressor(data_train, data_test):
    ...
    mean = data_train.groupby(['segment_id', 'vehicle_id'])['dwell_time'].mean()
...
def random_forest(X_train, X_test, y_train): 
    ...
    with open(f"/data/processed/{city}_baselines/best_params_rf_dwell_time.json", "r") as read_file:
...
def hyperparameter_random_forest(): 
    ...
    #save tp json
    with open(f"/data/processed/{city}_baselines/best_params_rf_dwell_time.json", "w") as outfile:
...
def hyperparemter_xgboost(X_train, y_train):
    ...
    #save tp json
    with open(f"/data/processed/{city}_baselines/best_params_xg_dwell_time.json", "w") as outfile:
        json.dump(best_params, outfile)
...
def xgboost(X_train, X_test, y_train, y_test):
    ...
    with open(f"/data/processed/{city}_baselines/best_params_xg_dwell_time.json", "r") as read_file:
        params = json.load(read_file)
        param.update(params)
...
if __name__ == "__main__":
    ...
    X_train = np.load(f"/data/processed/{city}_baselines/X_train_dwell_time.npy", allow_pickle=True)
    X_test = np.load(f"/data/processed/{city}_baselines/X_test_dwell_time.npy", allow_pickle=True)
    y_train = np.load(f"/data/processed/{city}_baselines/y_train_dwell_time.npy", allow_pickle=True)
    y_test = np.load(f"/data/processed/{city}_baselines/y_test_dwell_time.npy", allow_pickle=True)

    data_train = pd.read_csv(f"/data/processed/{city}_baselines/data_train_dwell_time.csv")
    data_test = pd.read_csv(f"/data/processed/{city}_baselines/data_test_dwell_time.csv")
    ...
    if not os.path.isfile(f"/data/processed/{city}_baselines/best_params_rf_dwell_time.json"):
    ...
    if not os.path.isfile(f"/data/processed/{city}_baselines/best_params_xg_dwell_time.json"):
        print("No best params for xgboost found...creating params")
    ...
    data_pred.to_csv(f"/data/processed/{city}_baselines/data_pred_dwell_time.csv", index=False) 
